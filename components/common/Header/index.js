...
import { FaBarsStaggered, FaCaretDown, FaCircleUser, FaFlagCheckered } from "react-icons/fa6";
  ...
  const handleSearchChange = async (event) => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}course/search?query=${value}`,
        {
          headers: {
            "X-API-Key": xApiKey,
          },
      ...
      <div className="px-6 md:px-16 py-2 bgBlueDark">
        <div className="flex justify-between flex-wrap">
          <ul className="flex gap-6 flex-wrap">
            ...
            <li className="text-white flex gap-3 text-sm  items-center">
              +1 833 564 3321 {!saleisActive ? <span>(Toll Free)</span> : <span className="cursor-pointer" onClick={() => showModels(true)}><FaCaretDown /></span>}
      ...
      <header className="border-b flex justify-between items-center px-6 py-3 md:px-16 md:py-3 bg-white w-full">
                  ...
                  <div className="flex-1 border rounded-md p-4 max-w-sm">
                    ...
                    <div className="flex items-center text-sm text-gray-600 mb-2">
                      <span className="flex items-center mr-4">
                        ...
                        <span className="text-blue-600 font-medium">{hoveredItem?.liveCourses?.learners} Learners</span>
                      </span>
        ...
        <div className="flex gap-5 items-center">
          ...
          <div className="hidden lg:flex gap-3">
            <Link href="/signup" className="text-base whitespace-nowrap px-3 py-1 rounded textBlueDark border-2" style={{ borderColor: "#005483" }}>Sign Up</Link>
            <Link href="/login" className="text-base border-2 whitespace-nowrap px-3 py-1 rounded textBlueDark" style={{ borderColor: "#005483" }}>Log in</Link>
          </div>
        </div>
      </header>
    </>
